...
class VideoLabelandprediction(QWidget):
    ...
    #TODO: modify it to make it recover from original, not just the last one   
    def read_recovery_from_idx(self, idx ):
        if len(self.entries):
            self.keywords_origin = self.entries[idx]['keywords'].copy()
            self.prediction_origin = self.entries[idx]['predictions'].copy()
        print(self.keywords_origin)
        print(type(self.keywords_origin))
        print(self.prediction_origin)
        print(type(self.prediction_origin))
    ...
    def init_shot_bar(self):
        ...
        for i in range(len(self.entries)):
            ...
            button.clicked.connect(partial(self.read_recovery_from_idx, i))
        # self.scroll.show()
    ...
    def init_shot_info(self, j):
        ...
        for i in range(len(predictions)):
            button = QPushButton(predictions[i])
            grid.addWidget(button, i+1, 1)
            self.buttons_predictions.append(button)
            button.clicked.connect(partial(self.delete_predictions, j, predictions[i]))
            button.clicked.connect(partial(self.change_color, button))
    ...
    def delete_predictions(self, index, prediction_name):
        # self.entries[index]['keywords'] = self.entries[index]['keywords'].remove(keyword_name)
        if prediction_name in self.entries[index]['predictions']:
            self.entries[index]['predictions'].remove(prediction_name)
        else:
            self.entries[index]['predictions'].append(prediction_name)
    ...
    def copy_former_one(self):
        if self.shot_idx>0:
            ...
            self.entries[self.shot_idx]['predictions'] = self.entries[self.shot_idx - 1]['genres'].copy()
    ...
    def recovery(self):
        ...
        self.entries[self.shot_idx]['predictions'] = self.prediction_origin.copy()
        self.init_shot_info(self.shot_idx)
